...
```{r}
ggplot(DJLeMahieu, aes(x = release_speed)) + # this one displays counts
  geom_freqpoly(aes(color = pitch_type), binwidth = 5, linewidth = 0.5)
```

```{r}
ggplot(DJLeMahieu, aes(x = bb_type, y = strikes)) +
  geom_boxplot()

ggplot(DJLeMahieu, aes(x = bb_type, y = balls)) +
  geom_boxplot()
```
